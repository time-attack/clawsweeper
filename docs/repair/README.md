...
Target validation runs through a versioned staged proof plan instead of artifact order. The plan parses only allowlisted read-only commands, rejects snapshot/formatter mutation flags, retains every required artifact/profile command, deduplicates exact argv, and orders repository integrity, static analysis, focused tests, the repository's canonical changed-surface gate, and broad/live/e2e proof from cheap to expensive. Narrow source/test repairs run focused tests early; migration, schema, security, workflow, dependency, and broad-surface repairs retain stronger supplied commands. Execution fails fast on a failed prerequisite, a canonical changed-surface stall, or a command that finishes beyond the total proof budget. A later command is skipped only when `config/target-repositories.json` carries an exact toolchain subsumption contract; arbitrary tests are never inferred redundant, QA/live proof is never subsumed, and required changed-surface, security, review, exact-head, GitHub-check, and live-proof gates remain intact.
