...
- Added immutable, privacy-bounded action ledger shards for all six ClawSweeper action families, with safe-integer JSON identity hashing, credential-alias rejection, parent-chain-stable no-follow writes, replay-stable run partitions, full producer binding, schema/runtime parity, and deterministic causal ordering.
